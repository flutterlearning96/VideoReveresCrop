...
@objc protocol PhotosLibraryVCDelegate {
    func didUpdateSelectedImages(images: [UIImage])
}
class PhotosLibraryVC: UICollectionViewController {
    
    weak var photosLibraryVCDelegate: PhotosLibraryVCDelegate?
    ...
    func updateUI() {
        ...
        rightBarButton = UIBarButtonItem(title: "Done", style: .done, target: self, action: #selector(startCollage))
            ...
            imageManager.requestImage(for: asset, targetSize: PHImageManagerMaximumSize, contentMode: .default, options: nil, resultHandler: { image, _ in
                ...
                if self.imageArray.count == self.selectedAssetArray.count {
                    OperationQueue.main.addOperation({                        
                        self.photosLibraryVCDelegate?.didUpdateSelectedImages(images: self.imageArray)
                    })
                }
            })
        }
...
extension PhotosLibraryVC: PHPhotoLibraryChangeObserver {
